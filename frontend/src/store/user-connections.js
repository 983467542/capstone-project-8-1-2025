...
export const fetchAllConnections = () => async (dispatch) => {
    ...
    try {
        ...
        console.error('Error:', err);
        dispatch(setError('Unable to fetch connections.'));
    } finally {
        dispatch(setLoading(false));
    }
};
...
export const addConnection = (connectionData, onError) => async (dispatch, getState) => {
    ...
    try {
        ...
        dispatch(getConnection(connectionData.user2Id));
        dispatch(setError(null));

    } catch (err) {
        const errorMsg = 'A connection now exists.';
        if (onError) onError(errorMsg);
        console.error(errorMsg);
    }
};
...
export const updateConnectionStatus = (connectionId, status) => async (dispatch) => {
    ...
    try {
        ...
        dispatch(setError('Unable to update connection status.'));
        throw err;
    } finally {
        dispatch(setLoading(false));
    }
...
export const updateMeetingStatus = (connectionId, meetingStatus) => async (dispatch) => {
    ...
    try {
        ...
        dispatch(setError('Unable to update meeting status.'));
    } finally {
        dispatch(setLoading(false));
    }
};

export const updateFeedback = (connectionId, feedback, field = null) => async (dispatch) => {
    dispatch(setLoading(true));
    try {
        const body = field
            ? { [field]: feedback }
        ...
        console.error('Error:', err);
        dispatch(setError('Unable to update feedback.'));
    } finally {
        dispatch(setLoading(false));
    }
};

export const updateConnection = (connectionData, onError) => async (dispatch) => {
    if (!connectionData || !connectionData.id) {
        const errorMsg = 'Connection ID is missing.';
        if (onError) onError(errorMsg);
        console.error(errorMsg);
        return;
    }
    try {
        const res = await csrfFetch(`/connections/${connectionData.id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(connectionData),
        });
        ...
        if (!res.ok) {
            const errorData = await res.json();
            const errorMsg = errorData.message || 'Failed to update connection.';
            if (onError) onError(errorMsg);
            console.error(errorMsg);
            return;
        }
...
export const removeConnection = (connectionId) => async (dispatch) => {
    ...
    try {
        ...
        console.error('Error:', err);
        dispatch(setError('Unable to delete connection.'));
    } finally {
        dispatch(setLoading(false));
    }
};
...
export const fetchFilteredResults = () => async (dispatch) => {
    ...
    try {
        ...
        if (!res.ok) {
            ...
            if (!allConnectionsRes.ok) throw new Error();
        ...
        dispatch(setError('Unable to fetch filtered results.'));
    } finally {
        dispatch(setLoading(false));
    }
};
