...
const GuessingGame = () => {
  ...
  return (
    <div>
      ...
      <div className="guessing-game-cards">
        {showPlayerForm && (
          <div className="player-setup">
            ...
            <form onSubmit={handleAddPlayer}>
              <input
                type="text" className="guessing-game-input"
                value={newPlayerName}
              ...
              {players.length >= 2 && (
                <button type="button" className={`rounded-rectangular-button ${theme}`} onClick={startGame}>
                  Start Game
                </button>
              )}
              ...
              <button className={`game-play-rounded-rectangular-button ${theme}`}
                onClick={() => alert("Feature coming soon")}
              >
                Load Game by Username
              </button>

              <button type="submit" className={`rounded-rectangular-button ${theme}`}>Add Player</button>
            </form>

            {players.length > 0 && (
              <div>

                <button type="button" className={`rounded-rectangular-button ${theme}`} onClick={() => setShowRemovePlayerForm(!showRemovePlayerForm)}>
                  Remove Player
                </button>
                ...
                {showRemovePlayerForm && (
                  <form onSubmit={handleRemovePlayer}>
                    ...
                    <button type="button" className={`rounded-rectangular-button ${theme}`}>Remove</button>
                  </form>
                )}
              </div>
            )}

            <button type="button" className={`rounded-rectangular-button ${theme}`} onClick={() => setShowRules(!showRules)}>
        ...
        {gameStarted && (
          <div className="game-play">
            <div className="game-header">
              ...
              <p>
                <strong>Current Player: </strong>{players[currentPlayer]}</p>
              <p><strong>Cards Remaining: </strong>{remainingCards} / {totalCards}</p>

              <button type="button" className={`rounded-rectangular-button ${theme}`} onClick={toggleRoastMode}>
            ...
            {selectedCategory && !selectedCard && (
              <div className="card-selection">
                ...
                <div className="cards-grid">
                  {allCards[selectedCategory].map((card) => (
                    <button
                      ...
                      className={`rounded-rectangular-button ${theme}`} disabled={usedCards.includes(`${selectedCategory}:${card.prompt}`)}
                    >
                      {card.prompt}
                    </button>
                  ))}
                ...
                <button type="button" onClick={handleGoBack} className={`rounded-rectangular-button ${theme}`} >
                  Back to Categories
                </button>
              </div>
            )}
            ...
            {selectedCard && !showPrompts && (
              <div className="card-guessing">
                ...
                <button type="button" onClick={() => setSelectedCard(null)} className={`rounded-rectangular-button ${theme}`} >
            ...
            {showPrompts && (
              <div className="discussion-prompts">
                ...
                <div className="match-controls">
                  {!isMatch && !forcedMatch && (
                    <button type="button" onClick={forceMatch} className={`rounded-rectangular-button ${theme}`} >
                      Make it a Match
                    </button>
                  )}
                  {forcedMatch && (
                    <button type="button" onClick={undoMatch} className={`rounded-rectangular-button ${theme}`} >
                      Undo Match
                    </button>
                  )}
                  {isMatch && !forcedMatch && (
                    <button type="button" onClick={() => setIsMatch(false)} className={`rounded-rectangular-button ${theme}`} >
                      Undo Match
                    </button>
                  )}
                </div>
                ...
                <div className="navigation-buttons">
                  <button type="button" onClick={resetGamePlay} className={`rounded-rectangular-button ${theme}`} >
                    Try Again
                  </button>
                  ...
                  <button type="button" onClick={nextPlayerTurn} className={`rounded-rectangular-button ${theme}`} >
                    Next Player&apos;s Turn
                  </button>
                </div>
              </div>
        ...
        {showSettings && (
          <div className="settings-panel">
            ...
            <div className="player-management">
              ...
              <button type="button" className={`rounded-rectangular-button ${theme}`} onClick={() => setShowAddPlayerForm(!showAddPlayerForm)}>
              ...
              {showAddPlayerForm && (
                <form onSubmit={handleAddPlayer}>
                  <input
                    ...
                    placeholder="Enter player name"
                  />
                  <button type="submit" className={`rounded-rectangular-button ${theme}`} >Add</button>
                </form>
              )}
                ...
                <>
                  ...
                  {showRemovePlayerForm && (
                    <form onSubmit={handleRemovePlayer}>
                      ...
                      <button type="submit" className={`rounded-rectangular-button ${theme}`} >Remove</button>
                    </form>
                  )}
                </>
              )}
            ...
            <div className="game-progress">
              <h4>Game Progress</h4>
              <p><strong>Round: </strong>{roundNumber - 1}</p>
