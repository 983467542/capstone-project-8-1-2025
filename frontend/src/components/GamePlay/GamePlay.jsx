...
const traitCategories = [
  ...
  {
    category: 'Behavioral and Social Preferences',
    traits: ['Small group gatherings vs. Big parties',
      'Loud in group chats vs. Silent in group chats',
      'Loves to travel vs. Homebody on vacation',
      'Needs quiet time vs. Always surrounded by noise',
      'People watcher vs. Talker',
      'Social butterfly vs. Low-key',
      'Likes being the center of attention vs. Prefers to stay in the background',
      'Gives a lot of hugs vs. Not a hugger',
      'Always early vs. Always running late',
      'Will start dancing in public vs. Keeps it cool on the dance floor']
  },
  {
    category: 'Hobbies and Interests',
    traits: ['Art lover vs. Tech geek',
      'Loves reading vs. Loves podcasts',
      'Fitness junkie vs. Netflix binger',
      'Gamer vs. Non-gamer',
      'Into crafts/DIY vs. Prefer ready-made stuff',
      'Loves cooking vs. Takes-out more than cooks',
      'Outdoor activities vs. Indoor activities',
      'Sci-fi fan vs. Fantasy fan',
      'Fan of comics vs. Fan of anime/manga',
      'Enjoys puzzles vs. Enjoys board games']
  },
  {
    category: 'Food and Drink Preferences',
...
const GamePlay = () => {
  ...
  const [, setInteractionType] = useState('guessing');
  ...
  const getPlayersArray = (playersData) => {
    ...
    if (typeof playersData === 'string') {
      try {
        ...
        console.error('Failed to parse players data:', playersData);
        return [];
      }
    }
    return [];
  ...
  const players = getPlayersArray(game?.players);
  const currentPlayerIndex = game?.currentPlayerIndex ?? 0;
  const currentPlayerName = players?.[currentPlayerIndex] ?? 'Unknown';
  const currentGuesserIndex = currentPlayerIndex === 0 ? 1 : 0;
  const currentGuesserName = players?.[currentGuesserIndex] ?? 'Unknown';
  const roundNumber = game?.roundNumber ?? 0;
  const usedCards = game?.usedCards || [];
  const completedTraits = game?.completedTraits || [];
  ...
  useEffect(() => {
    if (!sessionUser) dispatch(restoreUser());
  ...
  const handleCategorySelect = (category) => {
    ...
    if (game?.gameSessionId) {
      dispatch(updateGameTrait(game.gameSessionId, {
  ...
  const handleOptionSelect = async (option) => {
    ...
    if (game?.gameSessionId) {
      try {
        await dispatch(updateGuessedValue(game.gameSessionId, option));
      } catch (error) {
        console.error('Failed to update guessed value:', error);
        setErrorMessage('Failed to save your guess. Please try again.');
      }
    }
  };
  ...
  const handleForceMatch = () => {
    if (!game?.gameSessionId) return;
    setOriginalMatchState(game.isCorrect);
    setForcedMatch(true);
    dispatch(updateGameCorrectness(game.gameSessionId, { isCorrect: true }));
  };
  ...
  const handleUndoMatch = () => {
    ...
    if (forcedMatch) {
      ...
      dispatch(updateGameCorrectness(game.gameSessionId, { isCorrect: originalMatchState ?? false }));
      setOriginalMatchState(null);
    } else {
      dispatch(updateGameCorrectness(game.gameSessionId, { isCorrect: false }));
    }
  };
  ...
  const handleChangeInteractionType = (type) => {
    ...
    if (
      ...
      setErrorMessage('Please set or undo the match before selecting an interaction type.');
      return;
    }

    setErrorMessage('');
    setInteractionType(type);
    dispatch(updateGameInteractionType(game.gameSessionId, { interactionType: type }));
  };

  const nextPlayerTurn = () => {
    if (!game?.gameSessionId || players.length === 0) return;

    const currentInteractionType = game?.interactionType || 'guessing';

    if (
      guessedValue === null ||
      game.isCorrect === null ||
      currentInteractionType === 'guessing'
    ) {
      setErrorMessage('Please set or undo the match and choose an interaction type before continuing.');
      return;
    }
    ...
    if (isLastPlayer && selectedTrait) {
      const newCompletedTraits = [...completedTraits, selectedTrait];
      const newUsedCards = usedCards.includes(selectedTrait)
        ? usedCards
        : [...usedCards, selectedTrait];

      updates = {
        ...updates,
        completedTraits: newCompletedTraits,
    ...
    dispatch(updateGameState(game.gameSessionId, updates));
  };
  ...
  const handleEndGame = async () => {
    if (!game?.gameSessionId) {
      console.error('Game ID is undefined. Cannot end game.');
      return;
    }
    ...
    try {
      await dispatch(endGame(game.gameSessionId));
      navigate('/game-start');
    } catch (error) {
  ...
  const handleResetGame = async () => {
    if (!window.confirm("Are you sure you want to reset the current game?")) return;

    try {
      setErrorMessage('');

      if (game?.gameSessionId) {
        await dispatch(resetFullGame(game.gameSessionId));
...
// )} */}
        ...
        <div className="game-controls">
          ...
          <button
            type="button"
            onClick={handleEndGame} className={`game-play-rounded-rectangular-button ${theme}`}>
            End Game
          </button>
          {/* )} */}
        ...
        <button className={`game-play-rounded-rectangular-button ${theme}`}
        ...
        <button className={`game-play-rounded-rectangular-button ${theme}`}
          onClick={() => alert("Feature coming soon")}
        >
          Add New Talk About Prompt
        </button>

        {showRules && (
          <div className="rules-section">
            <h3>How to Play</h3>
            <p>The game consists of six themed categories: Personality and Traits; Food and Drink; Music, Movies, and TV Shows; Travel and Adventure; Conversation Style and Humor; and Mindset and Lifestyle. Each category has a set of possible traits, preferences, or interests for each of you to guess about your match. You will be presented with two options for each category. At the start of the game, you both have the option to activate Roast Me Mode. If both of you opt in, a lighthearted, sarcastic twist will be added to the game. When Roast Mode is on, each guess is followed by a playful roast that adds humor to the conversation. These roasts are meant to be light-hearted and funny, not mean-spirited. After both of you make your guess, the game will reveal whether each guess was correct or incorrect. For correct guesses, a playful or humorous response will appear. For incorrect guesses, the game might make a sarcastic comment or ask a follow-up question to spark conversation. After each guess, you will receive &quot;Talk About This&quot; prompts, which are designed to spark deeper or more playful conversations.</p>
          </div>
        )}
        ...
        {showSettings && (
          <div className="settings-panel">
            ...
            <div className="game-progress">
              ...
              <p><strong>Current Player:</strong> {currentPlayerName || 'Unknown'}</p>
              <p><strong>Current Guesser:</strong> {currentGuesserName || 'Unknown'}</p>
              <p><strong>Cards Used: </strong>{usedCards.length} / {totalCards}</p>
              <p><strong>Cards Remaining: </strong>{remainingCards}</p>
            </div>
          </div>
        )}
          ...
          <div className="active-game">
                ...
                {traitCategories.map((categoryObj, index) => (
                  <div key={index} className="category-section">
                    ...
                    <div className="traits-list">
                      <button
                        type="button"
                        className={`game-play-rounded-rectangular-button ${theme}`} onClick={() => handleCategorySelect(categoryObj.category)}
                        disabled={loading}
                      >
                      ...
                      {selectedCategory === categoryObj.category && !selectedTrait && categoryObj.traits.map((trait, i) => (
                        <button
                          ...
                          className={`game-play-rounded-rectangular-button ${theme} ${selectedTrait === trait ? 'selected' : ''} ${completedTraits.includes(trait) ? 'completed' : ''}`}
                        >
            ...
            {selectedTrait && !guessSubmitted && (
              <div className="trait-options">
                <h3>Which option best describes {currentPlayerName}?</h3>
                <div className="options-buttons">
                  {getTraitOptions(selectedTrait).map((option, index) => (
                    <button
                      type="button"
                      key={index}
                      className={`game-play-rounded-rectangular-button ${theme} ${guessedValue === option ? 'selected' : ''}`}
            ...
            {guessSubmitted && (
              <div className="post-guess-actions">
                ...
                <div className="interaction-type-buttons">
                  ...
                  <button
                    type="button"
                    className={`game-play-rounded-rectangular-button ${theme} ${game.interactionType === 'guessing' ? 'active' : ''}`}
                  ...
                  <button
                    type="button"
                    className={`game-play-rounded-rectangular-button ${theme} ${game.interactionType === 'roasts' ? 'active' : ''}`}
                  ...
                  <button
                    type="button"
                    className={`game-play-rounded-rectangular-button ${theme} ${game.interactionType === 'talk-about' ? 'active' : ''}`}
                ...
                <button
                  type="button"
                  onClick={resetGamePlay} className={`game-play-rounded-rectangular-button ${theme}`}>
                  Undo Guess
                </button>
                <button
                  type="button"
                ...
                <button
                  type="button"
                  onClick={nextPlayerTurn} className={`game-play-rounded-rectangular-button ${theme}`}>
                  Next Player&apos;s Turn
                </button>
              </div>
            )}
